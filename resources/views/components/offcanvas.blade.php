    ...
    <div class="mt-3">
          ...
          <div class="offcanvas offcanvas-end" data-bs-scroll="true" tabindex="-1" id="offcanvasBoth" aria-labelledby="offcanvasBothLabel">
                <div class="offcanvas-header">
                    {{-- {{dd($reservetions[0]->date)}} --}}
                    {{-- {{ \Carbon\Carbon::parse($reservetions[0]->date)->format('d-m-Y') }} --}}
                      ...
                      <h5 id="offcanvasBothLabel" class="offcanvas-title">{{count($reservetions) > 0 ?  date('d-m-Y', strtotime($reservetions[0]->date))  : ''}} օրվա գրանցումները</h5>
                        {{-- <h5 id="offcanvasBothLabel" class="offcanvas-title">{{count($reservetions) > 0 ?  \Carbon\Carbon::parse($reservetions[0]->date)->format('d-m-Y') : ''}} օրվա գրանցումները</h5> --}}
                                ...
                                <ul class="list-group">
                                    @foreach ($reservetions as $key => $item)
                                        {{-- <div class="card accordion-item"> --}}
                                                ...
                                                <li class="list-group-item list-group-item-{{$item->type=="enter" ? "success" :"danger"}}">{{date('H:i', strtotime($item->date))}}-{{$item->type=="enter" ? "Մուտք" :"Ելք"}}</li>


                                            {{-- <h2 class="list-group-item-primary"> {{date('H:i', strtotime($item->date))}}-{{$item->type=="enter" ? "Մուտք" :"Ելք"}}</h2> --}}
